...
- **Transient State Architecture** (`docs/architecture/transient-run-state.md`, `docs/architecture/valkey-keyspace.md`, `internal/casino`, `internal/pvp`, `internal/gvg`): ✅ Ephemeral Turn & Session Lobby Architecture (Candidate C) standardized across multiplayer domains (Party, PvP, GvG, Casino) with authentic 1800s sliding TTL, `party2:<domain>:rooms:active` ZSet index scored by `UpdatedAt.Unix()`, lazy `ZREMRANGEBYSCORE` pruning, and Two-Phase Settlement into MariaDB Master; implemented `ValkeyRoomRepository` for Casino multiplayer room and turn state, wired in `cmd/party2`, and legacy relational tables dropped via Migration 085 (#635); standardized PvP and GvG room repositories to Candidate C keyspace, `UpdatedAt` scoring, and lazy TTL pruning (#644). In-Progress Run Buffers (Candidate D: Dungeon/Challenge) and Shared Boss HP (Candidate E) verified and active.
...
- **Guild** (`internal/guild`, `internal/api/http`): ✅ Foundation (5,000G), dynamic Guild Points (`gpoint`) across social/combat hooks (tavern meals +2 GP, job change +50 GP, contest, GvG, callout +1 GP) and server rankings, custom member role titles (up to 6 full-width characters via `あたえる`), hex color customization (`からー`) with server-wide uniqueness and GvG eligibility validation, formal membership application & approval gating (`参加申請中`, `あたえる` approval, `追放` rejection/expulsion with notification letters), broadcast member callouts (`よびかける`, +1 GP), visual personalization (3,000G guild mark, catalog-priced wallpapers), daily scheduled 20-day inactivity automatic disbandment worker, full HTTP REST API endpoints (`GET`/`POST /guilds`, `GET`/`DELETE /guilds/{id}`, `POST` apply/approve/reject/callout/leave, `PUT` title/customization, `DELETE` kick, `GET /characters/{id}/guild`) with complete OpenAPI 3.1 specs; fictional gold-donation leveling and capacity scaling completely purged (#490, #591, #633).
- **Casino** (`internal/casino`): ✅ Multi-Player Room Lobby (2..8 players, Indian Poker, High-Low, Doppelganger, speed/rate/password/spectators) migrated to Valkey Master (`ValkeyRoomRepository`, Candidate C) with 1800s sliding TTL, active ZSet index, and Two-Phase Settlement, completely replacing legacy `casino_rooms`/`casino_members` relational tables (dropped in Migration 085, #635); atomic turn serialization and member mutation concurrency enforced via Valkey distributed room lock (`party2:casino:lock:room:<room_id>`) with Lua release script (`release_room_lock.lua`) and reentrant context execution, eliminating lost updates and pot race conditions under concurrent turns (#642); authentic 13-card forehead Indian Poker (`party2/lib/casino_indian.cgi`), authentic multi-player High-Low (`party2/lib/casino_highlow.cgi`), authentic multi-player Doppelganger (`party2/lib/casino_doppel.cgi`), 18 authentic prizes with atomic Depot auto-routing (`party2/lib/casino.cgi`), single deterministic `economy.TransactionRunner` exchange path eliminating legacy fallback and repository exchange methods (#453); one-way gold-to-coin exchange with divergent reverse exchange excised, slot authentic fatigue checks & miss penalty (+1 tired), Job 46 Gambler restriction on 200-coin slot, Wish 5 25% +50% payout bonus, multi-winner `VARCHAR(255)` schema parity, and 1800s idle room auto-purging (#630); fictional solo dealer poker, solo high-low, solo doppelganger and 52-card standard deck completely purged (#486, #590).
- **Lottery & Raffle** (`internal/lottery`): ✅ Server-wide 20-cap Takarakuji lottery (`party2/lib/takarakuzi.cgi`, NPC `@クラゲ`), pessimistic row lock concurrency serialization (`takarakuji_rounds` Rank 0 `FOR UPDATE`) preventing over-selling beyond 20 tickets, atomic drawing transaction (`RunInTx`) with premature drawing rejection (`ErrNotReadyToDraw`), deterministic ascending lock hierarchy (Rank 5) for depot delivery, and strict rollback on depot capacity overflow (`depot.ErrDepotFull`) preventing silent asset loss (#631); authentic Fukubiki raffle (`party2/lib/lot.cgi`, NPC `@フクスケ`) with 3-coupon Standard (stat seeds, weekday secret treasures, small medals) and 300-coupon Special (orbs, rare materials) draws with hand-occupancy depot automatic routing; fictional gold ticket purchase and gold prizes purged (#484, #485).
...
- **Event Plaza** (`internal/eventplaza`): ✅ Real-time plaza concurrency presence tracking (5-minute active window via Valkey Sorted Set + MariaDB), 26-item authentic merchant catalog at 3× markup across Tiers 1–3, active helper quest item exclusion, hand occupancy depot fallback delivery, and King Boss victory celebration banquets directly linked to Valkey presence (10/20/30 attendees unlocking merchant tiers) (#491, #634).
- **Secret Shop** (`internal/secretshop`): ✅ `job_lv >= 7` gate, 8 items at 3× price, Depot auto-delivery, puff-puff (dialogue only) (#462).
- **Tavern & Food Delivery** (`internal/tavern`): ✅ 14-item menu, HP/MP restore, fullness on counter meal, raffle tickets on counter meal, recurring standing order food delivery across repeated solo & party adventures without erroneous consumption, fullness mutation, or ticket mis-award, and automatic post-adventure fullness reset (`is_eat = 0`) allowing immediate dining (`bar.cgi`, `_battle.cgi`) (#475, #595, #634).
